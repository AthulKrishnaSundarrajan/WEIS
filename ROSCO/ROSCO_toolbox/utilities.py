...
def write_DISCON(turbine, controller, param_file='DISCON.IN', txt_filename='Cp_Ct_Cq.txt', rosco_vt = {}):
    ...
    file.write('{0:<12d}        ! VS_ControlMode	- Generator torque control mode in above rated conditions {{0: constant torque, 1: constant power, 2: TSR tracking PI control with constant torque, 3: TSR tracking PI control with constant power, 4: Fixed pitch variable speed PI torque controller}}\n'.format(int(rosco_vt['VS_ControlMode'])))
    ...
    file.write('{0:<12d}        ! GS_notch_n	- Number of notch filters to be placed on the generator speed\n'.format(rosco_vt['GS_notch_n']))
    file.write('{0:<12d}        ! TTM_notch_n	- Number of notch filters to be placed on the tower-top motion\n'.format(rosco_vt['TTM_notch_n']))
    file.write('{}      ! F_NotchCornerFreq_GS	- Natural frequency of the notch filters applied on the genrator speed signal [rad/s]\n'.format(''.join('{:<4.6f}  '.format(rosco_vt['F_NotchCornerFreq_GS'][i]) for i in range(len(rosco_vt['F_NotchCornerFreq_GS'])))))
    file.write('{}      ! F_NotchCornerFreq_TTM	- Natural frequency of the notch filters applied on the tower-top motion signal [rad/s]\n'.format(''.join('{:<4.6f}  '.format(rosco_vt['F_NotchCornerFreq_TTM'][i]) for i in range(len(rosco_vt['F_NotchCornerFreq_TTM'])))))
    file.write('{:<13.5f}       ! F_NotchCornerFreq_RBM	- Natural frequency of the notch filter on the blade root moment signals, [rad/s]\n'.format(rosco_vt['F_NotchCornerFreq_RBM']))
